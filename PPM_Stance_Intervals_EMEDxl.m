...
% Built for the EMED-xl platform.
clear, clc, close all
...
MPP = MPP(2:288,2:89); %Size based on total number of platform sensors
isZero = (MPP ~= 0);
...
fclose(fid);
NumberFrames = (LineCount - 303) / 301;
...
for i = 1:NumberFrames
    ...
    RawMatrix = zeros(288, 88); % Platform size
    for row = 1:288
        line = fgetl(fid);
        values = str2num(line);
        RawMatrix(row, :) = values(2:89);
    end
...
COP_idx_new(isnan(COP_idx_new)) = COP_idx_new(19);
